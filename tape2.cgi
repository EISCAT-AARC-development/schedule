...
def showloginName():
    ...
    if isAuthenticated():
        ...
        print("You are not authenticated", end=" ")
    print("</b>")
...
def showSummaries():
    ...
    total = sql.select_resource_storage("type = 'data' AND location LIKE %s", (url,), what="SUM(UNIX_TIMESTAMP(end)-UNIX_TIMESTAMP(start)) AS time, SUM(bytes) AS bytes")
    t = total[0]
    t.time /= 86400
    t.bytes /= 2**30
    ...
    t.time /= 86400
    t.bytes /= 2**30
    print(f"{t.time} days and {t.bytes} Gigabytes")
...
def search_by_date():
    ...
    try:
        ...
        print(f"Illegal date specification: {date}")
        return

    if antenna:
        if do_download:
            experiments = sql.select("experiments", experiment_name=exp_wo_antenna, antenna=antenna)
        else:
            experiments = sql.union_select("experiments", experiment_name=exp_wo_antenna, antenna=antenna)
    else:
        if do_download:
            experiments = sql.select("experiments", experiment_name=exp_wo_antenna)
        else:
            experiments = sql.union_select("experiments", experiment_name=exp_wo_antenna)
    ...
    print(f"<h2>The Data Archive has the following entries for data at  {date}: </h2>")
    if not isAuthenticated() and do_download:
        ...
        print("<input TYPE=BUTTON VALUE='Login' ONCLICK='javascript:loginUrl();'>", end=" ")
        print("</div>",)
    if do_download:
...
def search_by_tape():
    ...
    if not lines:
        print(f"Tape {tapeNumber} does not exist.")
        return
...
def search_by_experiment():
    ...
    print(f"Matching experiment {exp_pattern.replace('%','*')}", end=" ")
    print(f"and antenna {ant_pattern.replace('%','*')}")
...
def dump_lines_tape(lines):
    ...
    for d in lines:
        ...
        print(' '.join((d.type, '', d.start.strftime("%Y-%m-%d %H:%M:%S"), '', d.unix_end and d.end.strftime("%Y-%m-%d %H:%M:%S") or " "*19, '',
...
# Footer
print(f'<p align=center>This page prepared at {time.strftime("%H:%M UT %a %b %d, %Y")}</p>')
