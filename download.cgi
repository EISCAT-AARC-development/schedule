...
if len(locs) == 0:
    ...
    if len(exps.split()) == len(expsalt.split()):
        ...
        for expr in exps.split():
            if filename.count(expr) > 0:
                sel = expr
            j = 0
            for altexp in expsalt.split():
                if i == j and filename.count(altexp) > 0:
                    sel = expr
                j = j+1
            i = i+1
    for expr in exps.split():
    ...
    for expr in min1.split():
        if filename.count(expr) > 0: i = 1
    for expr in min2.split():
        if filename.count(expr) > 0: i = 2
    j = 0
