...
def update_database(k):
    ...
    if k.year == k.endyear and k.month == k.endmonth and k.day == k.endday:
        f.write('%s %s %s %s %s %s %c %s\t%s @%s\n'%(k.year, k.month, k.day, k.starttime, k.endtime, k.choice, status, k.experiment, k.resources, infofilename))
    else:
        # spans several days, make multiple entries in database
        f.write('%s %s %s %s 2400 %s %c %s\t%s @%s\n' % (k.year, k.month, k.day, k.starttime, k.choice, status, k.experiment, k.resources, infofilename))
        for day in range(int(k.day)+1, int(k.endday)):
            f.write('%s %s %02d 0000 2400 %s %c %s\t%s @%s\n'%(k.year, k.month, day, k.choice, status, k.experiment, k.resources, infofilename))
        f.write('%s %s %s 0000 %s %s %c %s\t%s @%s\n' % (k.year, k.month, k.endday, k.endtime, k.choice, status, k.experiment, k.resources, infofilename))
    f.close()
    ...
    try:
        ...
        print(f"Cannot write notes file: {why}")
        sys.exit()
    ...
    if k.description:
        f.write('description '+k.description+'\n')
